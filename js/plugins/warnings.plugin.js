...
export default class Warnings extends Plugin.Plugin {
    ...
    async init() {
        this.approveCount = await this.getSetting("approve_count", Plugin.InputType.Number);
    }
    async getArchiveChannel() {
        return await this.getSetting("archive_channel", Plugin.InputType.Channel);
    }
}
// ========== Functions ==========
...
function timeoutOrCancel(ans, channel, state) {
...
async function getWarningText(channel, state) {
    try {
 ...
 * @param user
 */
async function sendWarningTo(channel, warning, user) {
    try {
...
// ========== Sub Commands ==========
async function newWarning(message, state, approveCount) {
    let channel = message.channel;
...
async function cancelWarning(message, state) {
    message.channel.send(lang.canceled);
    resetWarning(state);
}
async function vetoWarning(message, state) {
    let channel = message.channel;
    let author = message.author;
    ...
    let vetos = state.read("warning", "vetos") || [];
...
async function approveWarning(message, state, approveCount) {
    let channel = message.channel;
    let author = message.author;
    ...
    let approves = state.read("warning", "approves") || [];
...
async function previewWarning(message, state) {
    let channel = message.channel;
...
async function sendWarning(message, state, client, plugin, approveCount) {
    let channel = message.channel;
    ...
    // Find target User
    let targetId = state.read("warning", "target") || "";
    let user;
    try {
    ...
    archiveChannel === null || archiveChannel === void 0 ? void 0 : archiveChannel.send(lang.archiveWarning(state.read("warning", "author"), targetId, approves));
    archiveChannel === null || archiveChannel === void 0 ? void 0 : archiveChannel.send(warning);
    // Reset Warning
    resetWarning(state);
}
